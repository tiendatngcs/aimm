...
def bar_plot(ax, data, lg, ll, title, colors=None, total_width=0.8, single_width=1, legend=True):
  ...
  colors = ['#e0f3db','#a8ddb5','#93db6c','#49bcc0','#43a2ca']
  # Number of bars per group
  n_bars = len(list(data)[0])

  # The width of a single bar
  #bar_width = total_width / (len(data)*n_bars*len(lg))
  ...
  for i, (name, values) in enumerate(data.items()):
    ...
    x_offset = bar_width*len(lg) * i + bar_width 
    x_ticks.append(x_offset+bar_width/2)  
    # Draw a bar for every value of that type
    for x, y in enumerate(values):
      bar = ax.bar(x*(len(lg)*len(lg)*len(lg)/(n_bars*n_bars*n_bars)) + x_offset, y, width=bar_width * single_width/(1 + 1/len(lg)), color=colors[x])
...
if __name__=='__main__':
  ...
  for n, bench in enumerate(col_title_list):
    ...
    '''
    assumption {baseline_bench1, tech1_bench1, tech2_bench1, ...,  
                baseline_bench2, tech1_bench2, tech2_bench2, ..., so on} 
                order while input files in the command line
    '''
    if(n%int(num_tech)==0): 
      b = bench.split('_')[0]
      print("n: ", n, "b: ",b)
  ...
  #fig, ax = plt.subplots()
  bar_plot(plt, data, lg, ll, title, total_width=1 + 2/(n_bars*10), single_width=.9)
