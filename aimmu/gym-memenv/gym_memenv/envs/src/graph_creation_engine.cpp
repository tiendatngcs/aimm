...
void GraphEntry::update_forward_acc_cnt(int s_cube, int tab_s_index){
  ...
  _tot_access++;
  _tot_acc_per_epoch++;
  //cout<<"tot acc: "<<_tot_access<<endl;
}

void GraphEntry::update_link(int s_cube, int tab_s_index){
  _inLinkList[s_cube]._weightedEdge[tab_s_index]++;
}
...
int Graph::get_index(unsigned long va){
  int index = (va>>11) % _num_entries;
  //cout<<"[GRAPH] va  "<<va<<" page : "<<(va>>11)<<" index: "<<index<<endl;
  //if(!_tab[index].isvalid()){
  //  index = -1;
  //}
  return index;//TODO neeed to have a more meaningful hash map
}

void Graph::set_root_info(int index, int pid, int node_id, unsigned long page){
  _tab[index].set_root_id_va_node(pid, node_id, page);
}
...
int Graph::findVictim(){//returns victim index
  ...
  _tab[_cir_q_fifo_ptr].makevalid();
...
void Graph::update_table(int curr_index, int s1_index, int s1_cube, int s2_index, int s2_cube){
    ...
    //cout<<"[GRAPH] current index: "<<curr_index<<endl;
...
void GraphEntry::get_acc_frac(vector<double>& vec){
  for(int i=0; i<_num_cubes; i++){
    int hop = hop_count(_cube_root, i);
    float hop_frac = (hop*1.0) / _max_hops;
    assert(_tot_access > 0);
...
void Graph::prep_input_for_node2vec(int start_index){
  ...
  input_for_node2vec.clear();
  //possible_migration_cand_list.clear();
  
  //#pragma omp parallel for num_threads(8) collapse(2)
  for(int i=0; i<_num_entries; i++){
    src = i;
    src_addr = _tab[i].get_page_root();
    cube = _tab[i].get_cube_root();
    pid = _tab[i].get_pid();
    ...
    for(int k=0; k<_num_entries; k++){
      ...
      _num_edges[i][k] = 0;
      dest = k;
      dest_addr = _tab[k].get_page_root();
      for(int j=0; j<_num_cubes; j++){
        ...
        if(tot_acc>0){ 
          l_w = (ew*1.0) / tot_acc;
          int hop = _tab[i].hop_count(src, dest);
          float hop_frac = (hop*1.0) / _max_hops;
          //l_w *= hop_frac;
          l_w = hop_frac;
        }
      ...
      if(_edge_w[src][dest] > 0){
        //input_for_node2vec.push_back(make_tuple((src*1.0)/_num_cubes, (dest*1.0)/_num_cubes, _edge_w[src][dest]));
        //input_for_node2vec.push_back(make_tuple(src, dest, _edge_w[src][dest]));
        input_for_node2vec.push_back(make_tuple(src_addr, dest_addr, _edge_w[src][dest]));
      }
      //_input_for_node2vec.push_back(tmp);
    }
    //possible_migration_cand_list.push_back(make_tuple(pid, cube, src_addr));
  }
  //node2vec_inp_display(0); 
}
...
/*
 * Input for node2vec is list of tuples as follows. 
...
bool Graph::ready2gen(int i){
  ...
  if(_tab[i].get_tot_acc() >= _threshold){
    cout<<"[GRAPH] "<<_tab[i].get_tot_acc()<<" "<<_threshold<<endl;
