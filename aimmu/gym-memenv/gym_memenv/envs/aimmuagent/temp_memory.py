...
class TempSequentialMemory(Memory):
    
    def __init__(self, limit, **kwargs):
        super(TempSequentialMemory, self).__init__(**kwargs)
    ...
    def can_get_state(self, cycle):
        #print(self.observations)
        if(self.observations != None):
          if(len(self.observations) > 2):
            state0 = self.observations[2]
            #print("current cycle: ",cycle, "cycle in state0: ", state0[512])
            #print(state0[512]) 
       
            #print("-----------------------------------------------------",state0)
            if ((cycle - state0[512]) > 512) and len(self.observations)>2:
              #print("-----------------------------------------------------",state0)
            #print("state0[512]: ", state0[512])
              return True
            else:
              return False

    def front(self, cycle):
      if(len(self.observations)>2):
        state0 = self.observations[2]
        ...
        if ((cycle - state0[512]) > 512):
          #print("state= ", state0, "action: ", action, "reward: ", reward, "state-1: ", state1, terminal1)
          return state0, action, reward, state1, terminal1
        else:
          assert (cycle - state0[512]) > 512
      else:
    ...
    def append(self, observation, action, reward, terminal, training=True):
        ...
        """ 
        super(TempSequentialMemory, self).append(observation, action, reward, terminal, training=training)
        
        # This needs to be understood as follows: in `observation`, take `action`, obtain `reward`
        # and weather the next state is `terminal` or not.
        if training:
          #print("======================================   appending in observation: ", observation)
            self.observations.append(observation)
            #print("observation structure: +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ ",self.observations)
    ...
    def get_config(self):
        ...
        # Returns
            Dict of config
        """
        config = super(TempSequentialMemory, self).get_config()
