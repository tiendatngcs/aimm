...
class AbstractDQNAgent(Agent):
    """Write me
    """
    def __init__(self, nb_actions, memory, gamma=.99, batch_size=16, nb_steps_warmup=10,
...
class AIMMUAgent(AbstractDQNAgent):
    """
    # Arguments
    ...
    """
    def __init__(self, env, model, policy=None, test_policy=None, enable_double_dqn=False, enable_dueling_network=False,
    ...
    def get_config(self):
      #config = super(DQNAgent, self).get_config()
        config = super(AIMMUAgent, self).get_config()
    ...
    def forward(self, observation):
        ...
        #print("state len: ", len(state), " Q len ", q_values)
        if self.training:
    ...
    def backward(self, reward, terminal):
        ...
        #self.temp_mem.append(self.recent_observation, self.recent_action, reward, terminal,
        #                       training=self.training)
        #cycle = self.recent_observation[512]
        #if(self.temp_mem.can_get_state(cycle)):#if true
        #    state0, action, reward, state1, terminal1 = self.temp_mem.front(cycle)   
        #    reward = self.env.get_reward(state0)
        #    self.memory.append(state0, action, reward, terminal, training=True)
        #    self.temp_mem.pop_front()
        ...
        #print("loss: ", metrics[0], " mae: ", metrics[1], " mean_q: ", metrics[2])
        #if(math.isnan(metrics[0])==False):
        #  self.loss_total += int(metrics[0])
        #  self.mae_total += int(metrics[1])
        #  self.mean_q_total += int(metrics[2])
        #  self.total_backprop += 1

        #  if(self.total_backprop%10000==0):
        #    avg_loss = self.loss_total / self.total_backprop
        #    avg_mae = self.mae_total / self.total_backprop
        #    avg_q = self.mean_q_total / self.total_backprop
        #    self.loss_total = 0
        #    self.mae_total = 0
        #    self.mean_q_total = 0
        #    self.total_backprop = 0
        #    self.loss_monitor.append(avg_loss)
        #    self.meanq_monitor.append(avg_q)
        #    self.env.plot_loss(self.loss_monitor)
        #    self.env.plot_meanq(self.meanq_monitor)
        #    print("avg_loss: ", avg_loss, " avg_mae: ", avg_mae, " avg_q: ", avg_q)
        #    #if(self.total_backprop%100000==0):
        #    #  self.model.save_weights("saved_weights/ai_mmu_memenv-v0_weights.h5f", overwrite=True)
